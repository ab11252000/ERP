(function() {
  ...
  window.getAuthDebugForCurrentPage = () => {
    const app = window.getFirebaseAppForCurrentPage?.();
    ...
    return {
      ...
      appCount: Array.isArray(window.firebase?.apps) ? window.firebase.apps.length : 0,
      appNames: Array.isArray(window.firebase?.apps) ? window.firebase.apps.map(item => item.name) : []
    };
  };
  ...
  function loadScript(src) {
    return new Promise((resolve, reject) => {
      const existing = document.querySelector(`script[data-auth-sdk-src="${src}"], script[data-sdk-src="${src}"]`);
      if (existing) {
