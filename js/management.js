(function() {
  ...
  function bindEvents() {
    ...
    btnClearOrders?.addEventListener('click', handleClearOrders);
  ...
  function handleOrderSubmit(event) {
    ...
    try {
      ...
      showToast(`已建立 ${newOrders.length} 筆訂單`);
      orderForm.reset();
      setDefaultCreatedDate();
      resetProductItems();
      switchView('orders');
  ...
  function renderStats() {
    ...
    if (!productionChart || !categoryChart) return;

    const completedOrders = orders.filter(order => order.status.stage === 'Completed');
    const byMonth = completedOrders.reduce((bucket, order) => {
      const date = order.dates.created || new Date();
      const key = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
      bucket[key] = (bucket[key] || 0) + window.utils.orderQuantity(order);
      return bucket;
    }, {});
    const byCategory = completedOrders.reduce((bucket, order) => {
      const key = getCategoryLabel(order.product.category);
      bucket[key] = (bucket[key] || 0) + window.utils.orderQuantity(order);
      return bucket;
    }, {});

    productionChart.innerHTML = renderSimpleList(byMonth, '目前沒有已完成資料');
    categoryChart.innerHTML = renderSimpleList(byCategory, '目前沒有已完成資料');
  }

  function renderSimpleList(source, emptyText) {
    const rows = Object.entries(source);
    if (!rows.length) return `<p class="text-muted text-center">${escapeHtml(emptyText)}</p>`;
